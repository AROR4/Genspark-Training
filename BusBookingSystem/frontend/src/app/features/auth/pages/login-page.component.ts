...
export class LoginPageComponent {
  ...
  submit(): void {
    ...
    this.authApi.login(this.form.getRawValue()).subscribe({
      next: response => {
        this.busy.set(false);
        const actualRole = this.authApi.role();

        if (actualRole === 'admin') {
          this.authApi.logout();
          this.message.set('Admin login is handled on a separate route.');
          this.toast.info('Admin login is available at /admin/login.');
          void this.router.navigate(['/admin/login']);
          return;
        }

        if (this.mode() === 'operator' || actualRole === 'operator') {
          this.toast.success('Operator login successful.');
          void this.router.navigate(['/operator/dashboard']);
          return;
        }

        this.message.set(`Logged in as ${response.role || 'user'}.`);
